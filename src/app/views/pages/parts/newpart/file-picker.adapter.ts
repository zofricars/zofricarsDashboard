...
export class DemoFilePickerAdapter extends FilePickerAdapter {
  ...
  public uploadFile(fileItem: FilePreviewModel) {
    ...
    const api = 'https://db.buckapi.com/api/containers/tixsImages/upload';
    const req = new HttpRequest('POST', api, form, {reportProgress: false});
    return this.http.request(req)
    .pipe(
      map( (res: HttpEvent<any>) => {
          if (res.type === HttpEventType.Response) {
          this._butler.partImages.push('https://db.buckapi.com:3333/api/server/local-storage/tixsImages/'+res.body.result.files.file[0].name);
